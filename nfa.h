...
/**
 * Return the array of next states specified by the given NFA's transition
...
extern void array_adder(int* a, int*b, int states);

extern int one_same_element(int* a, int*b, int states);
extern void array_printer(int* print, int states);
extern cDFA NFA_to_DFA(NFA nfa);
extern bool array_contains(int* array, int array_length, int element);

extern void reset_current_states(NFA nfa);
